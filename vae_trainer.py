...
class VAETrainer(object):
    ...
    def init_meters(self, args):
        ...
        self.meters['train_bleu'] = AverageMeter()
        self.meters['valid_loss'] = AverageMeter()
        self.meters['valid_nll_loss'] = AverageMeter()
        self.meters['valid_bleu'] = AverageMeter()
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            try:
                ...
                if state['args'].arch == self.args.arch:
                    self.get_model().load_state_dict(state['model'], strict=False if warmup_from_nmt else True)
                else:
                    # load pretrained encoder, decoder
                    load_pretrained_component_from_model(self.get_model().encoder, filename)
                    load_pretrained_component_from_model(self.get_model().decoder, filename)
            except Exception:
                raise Exception(
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        try:
            ...
            if 'bleu_score' in logging_output:
                self.meters['train_bleu'].update(logging_output.get('bleu_score', 0))
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        if 'bleu_score' in logging_output:
            self.meters['valid_bleu'].update(logging_output.get('bleu_score', 0))

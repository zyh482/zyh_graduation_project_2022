...
CUDA_VISIBLE_DEVICES=2 nohup python train.py $DATAPATH \
...
--criterion kl_label_smoothed_cross_entropy --max-update 150000 \
...
--hidden-dim $hidden_dim --bias-dim 512 --bias-gates 1 1 0 \
--eval-bleu \
--beam 5 --max-len-a 1.2 --max-len-b 10 \
--eval-bleu-remove-bpe | tee -a $SAVEDIR/training.log > $SAVEDIR/train.output &

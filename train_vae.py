...
# CREATED ON: 2022/4/7 8:48 PM
# LAST MODIFIED ON:
# AIM:
"""
Train model with VAE project on one or across multiple GPUs.
"""
...
def train_vae(args):
    ...
    # Only Train VAE_Project and Transformer-decoder
    ...
    # Build trainer
    trainer = VAETrainer(args, task, model, criterion)
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        for sample in progress:
            ...
            for k, v in log_output.items():
                if k in ['loss', 'nll_loss', 'ntokens', 'nsentences', 'sample_size', 'bleu_score']:
                    continue
                extra_meters[k].update(v)

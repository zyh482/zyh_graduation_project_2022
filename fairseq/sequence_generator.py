    ...
    def generate(
        ...
        **kwargs
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            attn_clone = attn.index_select(0, bbsz_idx.long())[:, :, 1:step+2] if attn is not None else None
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                sent = int(unfin_idx + cum_unfin[int(unfin_idx)])
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # )
            (_ignore, active_hypos) = torch.topk(active_mask, k=beam_size, dim=1, largest=False)
            ...
            # )
            tokens_buf[:, :step+1] = torch.index_select(tokens[:, :step + 1], dim=0, index=active_bbsz_idx)
            torch.gather(
